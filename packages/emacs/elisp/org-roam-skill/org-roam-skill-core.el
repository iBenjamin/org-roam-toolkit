...
(defun org-roam-skill--expand-filename (title)
  "Generate a filename for TITLE in timestamp-slug format.
Always produces YYYYMMDDHHMMSS-slug.org regardless of capture templates.
Slug: downcase, non-alphanumeric chars to underscore, collapse and trim."
  (let* ((slug (replace-regexp-in-string "[^[:alnum:]]" "_" (downcase title)))
