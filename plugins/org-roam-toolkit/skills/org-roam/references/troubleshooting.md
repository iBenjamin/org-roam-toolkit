...
1. Verify the elisp package is installed by Homebrew:
   ```bash
   ls "$(brew --prefix org-roam-toolkit)/libexec/packages/emacs/elisp/org-roam-skill/org-roam-skill.el"
   ```

2. `ortk-emacs-eval --pkg=org-roam-skill` resolves the elisp directory itself; no manual `load-path` is required for the toolkit's own bins. Only set `load-path` in your Emacs init if you want to use the elisp from inside Emacs directly:
   ```elisp
   ;; In your init.el (replace BREW_PREFIX with `brew --prefix org-roam-toolkit`)
   (use-package! org-roam-skill
     :load-path "BREW_PREFIX/libexec/packages/emacs/elisp/org-roam-skill")
